...
*Write here something about the confounders and the DAG* Based on the DAG model in Figure @fig{fig:dag}


_Popensity score Model_ 
In order to control for confounding (observed and unobserved), a propensity score model is developed with the 
...
data %>% 
  ggplot() + 
  aes(x=ps.score, color=as.factor(vacc), fill=as.factor(vacc)) + 
...
```{r figure1, fig.show='hold', fig.cap='Checking for non-positivity'}
library(ggplot2)
...
```{r}
t(aggregate(cbind(age, cvd, pulm, DM, contact) ~ vacc, data=data, mean))
#t(aggregate(cbind(age,meal.cal,wt.loss,sex=sex==2) ~ ph.ecog.bin,data=df,mean))  
# note: this works for sex as this is a binary variable (mean of such variable=proportion),
# sex==2 is Female
# if you have a categorical variable with more levels, you can use individual level indicators,
# eg cbind(..., "Never smoking"=smok==1,"Past smoking"=smok==2,"Current smoking"=smok==3,...) 

# PS via IPW adjusted baseline table:
#t(aggregate(cbind(age, cvd, pulm, DM, contact)*ipw ~ vacc,data=data,sum) 
...
The crude association between vaccination status and hospitalization was examined using logistic regression, and the odds ratio was found to be `r res_crude[1]` (95% CI: `r res_crude[2]`, `r res_crude[3]`), indicating a non-significant association. The C statistic,  a measure of discrimination, was `r res_crude_c`, suggesting poor predictive performance of the model. 
