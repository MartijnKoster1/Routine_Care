---
title: "Assignment Day 8"
author: "Martijn Koster 6234119, Jurrian van de Kraats 5961688, Tim Poorthuis 0651478"
date: "`r Sys.Date()`"
output:
...
<!-- Congrats You found our code, Enjoy!  -->
...
```{r codes, results='asis', fig.cap = ' ANAPHYL and GBS event codes'}
anaphyl <- codelist %>% 
...
The forest plot figures (see Figure \@ref(fig:t)), show the incidence rates. A total of `r TNE_anaphN + TNE_anaphP + TNE_gbs` events occurred. Of these events `r TNE_anaphN` were anaphylaxis narrow, `r TNE_anaphP` were anaphylaxis possible and `r TNE_gbs` were GBS. When stratified in to age bands, the person years become lower and there is low confidence. Especially in the high and low age bands. Most events occurred in the first three years. In these years each type of event status had around 15 to 20 occurred events. The last year of the study has almost no events and bad incidence rate confidence interval.


### Forest Plot Figures {-.tabset} 

#### Total  {-#tot}


```{r t, fig.cap='Forest plots of total IR'}
library(tidyverse)
...
```{r k, fig.cap= 'forest plots data from 2017'}
...
```{r i, fig.cap= 'Forest plots data from 2018'}
...
```{r o, fig.cap= 'Forest plots data from 2019'}
...
```{r 999, fig.cap= 'Forest plots data from 2020'}
...
We have learned to take data from various tables, and structure it to a dataset which can be analysed. In addition, we have learned that a lot of data is needed to make inference about incidence rates. That is, when data needs to be stratified. We discovered that the distribution of the data becomes skewed when stratified in different age bands. In this case the incidence over a 1000 person years confidence interval will get wider and thus make it harder to indicate the true incidence rate. More data from different sources could help in creating a better distributed stratification. This can be accomplished by using data from a Common Data Model. The data could also have some bias. All of the data is collected using health registries and (routine) care data. This means that every person in the dataset, has had contact with an health institution. Thus, perfectly healthy people with no events will not be included in the dataset. 
